...
//    private Map<String,Method> handlerMapping = new HashMap<String,Method>();
    ...
    /**
     * 使用对象的形式存放V2版本的 HandlerMapping  map集合，
     * 完美解决了V2版本的url,method 以外需要重新通过反射寻找对象，以及方法形参的下标问题，
     * 根据设计原则：单一原则，最少知道原则，帮助我们更好的理解
     *
     * Handler使用内部类存放即可（也可以是一个新对象，都可以）
     */
    ...
    private void doDispach(HttpServletRequest req, HttpServletResponse resp) throws Exception {
        ...
        for (Map.Entry<String, String[]> param : params.entrySet()) {
            //将当前参数的[]括号去掉，再将空格、换行、tab缩进等所有的空白替换成“,”
            String value = Arrays.toString(param.getValue()).replaceAll("\\[|\\]", "").replaceAll("\\s", ",");
            //判断当前参数名称在handler.paramIndexMapping中是否存在
            if(!handler.getParamIndexMapping().containsKey(param.getKey())){continue;}
            //通过请求参数名 获取参数paramIndexMapping数组的下标
            Integer index = handler.getParamIndexMapping().get(param.getKey());
            //使用convert进行类型转换,并将参数值存到请求的参数集合中
            paramValues[index] = convert(paramTypes[index], value);
        }
        //5.判断当前handler对象的handler.getParamIndexMapping()集合中是否存在request和response的参数的名称
    ...
    private Object convert(Class<?> type,String value){
        ...
        }else if(Double.class == type){     //判断是double  转double
    ...
    /**
     * 通过request请求获取url的请求路径
     * 然后再通过handlermapping获取对应的Handler对象
     * @param req
     * @return
     */
    private Handler getHandler(HttpServletRequest req) {
        ...
        for (Handler handler : this.handlerMapping) {
            //通过请求的url与handlermapping中Pattern的url进行判断
            Matcher matcher = handler.getPattern().matcher(url);
            //判断吻合，返回当前需要的handler对象
            if(matcher.matches()){
                return handler;
            }
        }
        ...
        for (Map.Entry<String, Object> entry : this.ioc.entrySet()) {
            ...
            for (Method method : clazz.getMethods()) {
                ...
                if(method.isAnnotationPresent(MHRequestMapping.class)){
                    ...
                    //添加url正则路径和method方法和当前method方法的对象
                    this.handlerMapping.add(new Handler(pattern,method,entry.getValue()));

                    System.out.println("mapping:"+url+","+method+","+entry.getValue());
    ...
    /**
     * 内部类，用于存放handlerMapping的url和方法映射，对象，等等参数，
     * initHandlerMapping初始化的存放对象
     *
     * 保存一个url和一个Method的关系+当前method方法存在的对象
     */
    public class Handler{
        ...
        /**
         * 提取方法中加入了注解的参数
         * 把方法中的注解参数拿到，获取一个二维数组
         * 因为一个方法中有多个参数，而一个参数上面又可以有多个注解
         * @param method
         */
        private void putParamIndexMapping(Method method) {
            ...
            for (int i = 0; i < parameterAnnotations.length; i++) {
                ...
                for (Annotation annotation : parameterAnnotations[i]) {
                   //判断当前注解是否是MHRequestParam注解 （一个参数有多个注解，但是当前我们只自己定义了MHRequestParam注解）
                    if(annotation instanceof MHRequestParam){
                        //获取注解名称：@MHRequestParam("name")  中的name
                        String paramName = ((MHRequestParam) annotation).value();
                        //判断注解名称不为空
                        if(!"".equals(paramName.trim())){
                            //添加注解名称和注解的下标到 this.paramIndexMapping 集合中
